...
<html lang="en">
  ...
  <body>
    ...
    <?php 
      $datos =  $reservaController->obtener_asientos(head($idVuelo));
    ?>
    ...
    <section class="features-icons bg-white text-center">
      <div class="container" style="margin-top: 20%">
        ...
        <form method="POST" action="{{ route('siguiente') }}">
          ...
          <input id= "idVuelo" name= "idVuelo" value = "{{$idVuelo}}" class="btn btn-primary" style="display:none">


         @foreach((array)$datos as $dato)
         <div class="container" style="margin-top: auto;">
            
              <input type="checkbox" name="vehicle[]" value="{{($dato->id)}}"> {{($dato->letra)}} {{($dato->mumero)}}<br>
